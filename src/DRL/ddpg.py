...
class DDPG(object):
    ...
    def high_freq_reward(canvas, gt):
        ...
        grad_canvas_x = F.conv2d(canvas, sobel_x, padding=1)
        grad_canvas_y = F.conv2d(canvas, sobel_y, padding=1)
        grad_canvas = torch.sqrt(grad_canvas_x**2 + grad_canvas_y**2)

        grad_gt_x = F.conv2d(gt, sobel_x, padding=1)
        grad_gt_y = F.conv2d(gt, sobel_y, padding=1)
        grad_gt = torch.sqrt(grad_gt_x**2 + grad_gt_y**2)

        # Calculate the L2 difference in high-frequency areas
        hf_diff = ((grad_canvas - grad_gt) ** 2).mean(dim=[1, 2, 3])
        return -hf_diff  # Negative for reward
