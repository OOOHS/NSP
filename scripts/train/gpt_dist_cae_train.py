...
from src.GPT.model_dist import GPTModelWithCAE, MDNLoss
...
class CelebADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        attribute = attribute.unsqueeze(0)  # 确保属性是 1x40 的张量
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, scaler, config, rank):
    ...
    for i, (attributes, sequence) in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast():  # 开启半精度计算
            pi, mu, full_cov, mean, var = model(attributes, sequence)
            target = sequence.to(pi.dtype)  # 确保目标数据类型与模型输出匹配
            mdn_loss = criterion(pi, mu, full_cov, target)

            # 计算 KL 散度损失
            # 假设 mean 和 var 的形状是 [batch_size, feature_dim]
            # 标准正态分布的 mean 和 var 是 0 和 1
            kl_loss = 0.5 * (var + mean**2 - 1 - torch.log(var)).mean()
            total_loss_batch = mdn_loss + config.kl_weight * kl_loss

        # 使用 GradScaler 进行梯度缩放
        scaler.scale(total_loss_batch).backward()  # 反向传播时缩放梯度
        ...
        if rank == 0:  # 只在 rank 0 上打印进度
            tqdm.write(f"Batch [{i + 1}/{num_batches}], MDN Loss: {mdn_loss.item():.4f}, KL Loss: {kl_loss.item():.4f}, Total Loss: {total_loss_batch.item():.4f}")
