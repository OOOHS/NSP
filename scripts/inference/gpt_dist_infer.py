...
# 控制采样动作的多样性，数值越大，采样的动作越多样化，但可能会导致生成的图像质量下降
TEMPATURE = 8e-2
...
parser.add_argument('--model_path', default='./model_checkpoints/model_epoch_1390_loss_8.5577.pth', type=str, help='model path')
...
def generate_sequence(model, start_attributes, seq_len, action_dim, device):
    ...
    with torch.no_grad():
        ...
        for step in range(seq_len):
            pi, mu, full_cov = model(start_attributes, seq[:, :step+1])
            ...
            # 从多元高斯分布中采样 # 控制协方差矩阵的数值大小，从而控制采样的动作的多样性
            mvn = MultivariateNormal(loc=cur_mu, covariance_matrix=cur_full_cov*TEMPATURE)
